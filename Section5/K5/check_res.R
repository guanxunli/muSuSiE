...
out_res <- readRDS(paste0("Section5/K5/results/pc_com", e_com, "pri", e_pri, ".rds"))
## check results
res <- list()
...
############################# GES method #############################
lambdas <- c(1, 2, 3, 4, 5)
out_res <- readRDS(paste0("Section5/K5/results/GES_com", e_com, "pri", e_pri, ".rds"))
## check results
res <- list()
for (iter_K in seq_len(K)) {
  res[[iter_K]] <- list()
...
############################# joint GES method #############################
lambdas <- c(1, 2, 3, 4, 5)
out_res <- readRDS(paste0("Section5/K5/results/jointGES_com", e_com, "pri", e_pri, ".rds"))
## check results
res <- list()
for (iter_K in seq_len(K)) {
  res[[iter_K]] <- list()
...
############################# JSEC #############################
out_res <- readRDS( paste0("Section5/K5/results/JESC_com",
                           e_com, "pri", e_pri, ".rds"))
## check results
res <- list()
for (iter_K in seq_len(K)) {
...
############################# MPenPC #############################
out_res <- readRDS(paste0("Section5/K5/results/MPenPC_com",
                           e_com, "pri", e_pri, ".rds"))
## check results
res <- list()
for (iter_K in seq_len(K)) {
...
for (iter_graph in seq_len(n_graph)) {
  alpha_mat_list <- out_res[[iter_graph]][[1]]
  for (iter_K in seq_len(K)) {
    alpha_mat_tmp <- alpha_mat_list[[iter_K]]
    adj <- ifelse(alpha_mat_tmp == TRUE, 1, 0)
    adj <- t(adj)
    g <- as(getGraph(adj), "graphNEL")
    adj_true <- t(graph_sim$G[[iter_graph]][[iter_K]])
    g_true <- as(getGraph(adj_true), "graphNEL")
