...
source("Section3/simulation_code/K2/mh_utility.R")
## simulation function
simu_vs_fun <- function(K, n, p, p_c, p_s, sigma, sigma0, prior_vec) {
  ...
  #### separate MH method
  index_res_mhs <- list()
  sens_res_mhs <- rep(NA, K)
  prec_res_mhs <- rep(NA, K)
  ## data set
  time1 <- Sys.time()
  for (k in seq_len(K)) {
    ...
    if (length(index_res_mhs[[k]]) == 0) {
      ...
      sens_res_mhs[k] <- length(intersect(
        index_res_mhs[[k]],
        c(index_s[[k]], index_c)
      )) / (p_c + p_s)
      prec_res_mhs[k] <- length(intersect(
        index_res_mhs[[k]],
        c(index_s[[k]], index_c)
      )) / length(index_res_mhs[[k]])
    }
  }
  time_mhs <- Sys.time() - time1
  #### joint MH method
  index_res_mhj <- list()
  ...
  prec_res_mhj <- rep(NA, K)
  for (k in seq_len(K)) {
    index_res_mhj[[k]] <- which(res[[k]] > 0.5)
    if (length(index_res_mhj[[k]]) == 0) {
      sens_res_mhj[k] <- prec_res_mhj[k] <- 0
    } else {
      sens_res_mhj[k] <- length(intersect(
        index_res_mhj[[k]],
        c(index_s[[k]], index_c)
      )) / (p_c + p_s)
      prec_res_mhj[k] <- length(intersect(
        index_res_mhj[[k]],
        c(index_s[[k]], index_c)
      )) / length(index_res_mhj[[k]])
    }
  }
    ...
    for (p_c in p_c_vec) {
      for (p_s in p_s_vec) {
        ...
        saveRDS(out_res, paste0(
          "Section3/simulation_code/K2/results/K", K,
