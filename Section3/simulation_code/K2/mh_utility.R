...
mh_joint <- function(dta_list, max_mcmc = 1e5, burn_in = 1e4,
  ...
  g <- p^(2 * kappa1) - 1
  # log likelihood
  loglike_fun <- function(X, Y, sum_Y, gamma_x) {
    # get parameters
    gamma_l1 <- sum(gamma_x)
  ...
  loglike_curr_vec <- numeric(K)
  for (iter_K in seq_len(K)) {
    loglike_curr_vec[[iter_K]] <- loglike_fun(
      dta_list[[iter_K]]$X,
      dta_list[[iter_K]]$Y,
      sum_list[[iter_K]],
  ...
  loglike_curr <- sum(loglike_curr_vec)
  
  ## log prior
  logprior_fun <- function(state_curr_list) {
    n_active_vec <- sapply(seq_len(p), function(i) {
      return(sum(unlist(lapply(state_curr_list, function(state_curr) {
        return(state_curr[i])
      }))))
    })
    n_active_vec <- n_active_vec + 1
    return(-sum(omega_use[n_active_vec] * log(p)))
  }
  logprior_curr <- logprior_fun(state_curr_list)
  ...
  for (iter_mcmc in seq_len(max_mcmc)) {
    ...
    loglike_prop_vec <- loglike_curr_vec
    index_prop <- sample(p, 1)
    ...
    for (iter_dta in dta_prop) {
      ...
      loglike_prop_vec[iter_dta] <- loglike_fun(
        dta_list[[iter_dta]]$X,
        dta_list[[iter_dta]]$Y,
        sum_list[[iter_dta]],
    ...
    loglike_prop <- sum(loglike_prop_vec)
    logprior_prop <- logprior_fun(state_prop_list)
    logpost_prop <- logprior_prop + loglike_prop
    accept_prob <- min(1, (exp(logpost_prop - logpost_curr)))
    ## accept or nor
    if (runif(1, 0, 1) < accept_prob) {
      state_curr_list <- state_prop_list
      loglike_curr_vec <- loglike_prop_vec
      logpost_curr <- logpost_prop
    }
    if (iter_mcmc > burn_in) {
      for (iter_K in seq_len(K)) {
